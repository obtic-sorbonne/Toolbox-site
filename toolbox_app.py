...
def load_text(source, encoding="latin-1"):
    """
    Charge un texte complet depuis une URL ou un fichier local.
    """
...
def load_lines(source, encoding="latin-1"):
    """
    Charge un fichier ligne par ligne depuis une URL ou un chemin local.
    Retourne un itérable sur les lignes.
    """
    text = load_text(source, encoding)
    return StringIO(text)  # Fournit une interface fichier ligne à ligne
...
def correction_graphie():
    form = FlaskForm()
    return render_template('outils/correction_graphie.html', form=form)
...
def sanitize_filename(filename):
    """Limite la longueur du nom de fichier et remplace les caractères spéciaux."""
    filename = filename[:255]  # Limite la longueur à 255 caractères
...
def tokenized_corpus_from_text(text):
    #Modifie la fonction de tokenisation pour accepter directement un texte brut.
    doc = text.replace("\n", " ")
    doc = re.sub(r"<[^>]+>", " ", doc)
...
@app.route('/normalisation_graphies', methods=["GET", "POST"])
def normalisation_graphies():
    ...
    for f in files:
        try:
            ...
            # Utilisation de la nouvelle fonction de tokenisation
            words_list = tokenized_corpus_from_text(input_text)
            ...
            with open(output_path, 'w', encoding='utf-8') as out:
                # Remplacer dans l'input_text chaque occurrence de la clé 'i' par la valeur 'y'
                modified_input_text = input_text
                for i, y in results.items():
                    modified_input_text = modified_input_text.replace(i, y)

                # Écrire le texte modifié dans le fichier
                out.write("The new text is:\n" + modified_input_text + "\n\n")

                # Écrire le dictionnaire traité dans le fichier
                out.write("The modified words are:\n")
                for i, y in results.items():
                    out.write(f"{i} : {y}\n")


        except UnicodeDecodeError as e:
            print(f"Erreur de décodage pour le fichier {f.filename}: {e}")
        except Exception as e:
            print(f"Une erreur est survenue lors du traitement du fichier {f.filename}: {e}")
        finally:
            f.close()
